...
function addTableProduct() {
  ...
  xhr.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      var list_products = JSON.parse(this.responseText);

      var tableBody = document.getElementById("yourTableBody");
  ...
  xhr.open("GET", "./AJAX/getData.php", true);
  xhr.send();
}
...
function addKhungSuaSanPham(masp) {
  ...
  for (var c of company) {
    if (sp.company == c) s += `<option value="` + c + `" selected>` + c + `</option>`;
  ...
  for (var i = 0; i < u.length; i++) {
    for (var j = 0; j < u[i].donhang.length; j++) {
      if (u[i].donhang[j].ngaymua == maDonHang) {
        if (duyetDon) {
          if (u[i].donhang[j].tinhTrang == "Đã thanh toán") {
...
function getValueOfTypeInTable_DonHang(tr, loai) {
  ...
  switch (loai) {
    case "stt":
      return Number(td[0].innerHTML);
    case "ma":
      return new Date(td[1].innerHTML);
    case "khach":
      return td[2].innerHTML.toLowerCase();
    case "sanpham":
...
function voHieuHoaNguoiDung(inp, taikhoan) {
  ...
  for (var u of listUser) {
    if (u.username == taikhoan) {
      ...
      setTimeout(
        () => alert(`${value ? "Khoá" : "Mở khoá"} tải khoản ${u.username} thành công.`),
        500
      );
      break;
    }
