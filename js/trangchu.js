window.onload = function () {
  ...
  for (var c of company) {
    addCompany("img/company/" + c + ".png", c);
  }
...
function themNutPhanTrang(soTrang, trangHienTai) {
  ...
  if (trangHienTai < soTrang) {
    divPhanTrang.innerHTML +=
      `<a href="` + k + `page=` + (trangHienTai + 1) + `"><i class="fa fa-angle-right"></i></a>`;
  }
}
...
function addAllChoosedFilter() {
  if (filtersFromUrl.company != "") addChoosedFilter("company", filtersFromUrl.company);
  if (filtersFromUrl.search != "") addChoosedFilter("search", '"' + filtersFromUrl.search + '"');
  if (filtersFromUrl.price != "") {
    var prices = filtersFromUrl.price.split("-");
    addChoosedFilter("price", priceToString(prices[0], prices[1]));
  }
  if (filtersFromUrl.promo != "") addChoosedFilter("promo", promoToString(filtersFromUrl.promo));
  if (filtersFromUrl.star != "") addChoosedFilter("star", starToString(filtersFromUrl.star));
...
function createLinkFilter(type, nameFilter, valueAdd) {
  ...
  if (o.sort.by != "") link += (h ? "&" : "?") + "sort=" + o.sort.by + "-" + o.sort.type;
